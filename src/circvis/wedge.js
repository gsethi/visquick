...
//    var value_key = data._value_key;
    ...
    if (data._draw_axes) {
        ...
        svg_obj.selectAll("svg.circvis")
            .data(y_linear.ticks(4))
            .enter().append("svg:g")
            .attr("class", "axes")
            .each(draw_axes);
...
vq.models.CircVisData.prototype._setupData = function() {
    ...
    chrom_keys_array.forEach(function(d) {
        ...
        chrom_groups.push({key:d, startAngle: startAngle[d], endAngle: startAngle[d] + 2 * Math.PI * normalizedLength[d], theta:theta[d] });
    });
